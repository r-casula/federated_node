...
class Keycloak:
    ...
    async def get_token(
        ...
        async with httpx.AsyncClient() as requests:
            response_auth: httpx.Response = await requests.post(
                URLS["get_token"],
                data=payload,
                headers={"Content-Type": "application/x-www-form-urlencoded"},
            )
    ...
    async def is_token_valid(
        ...
        if with_permissions:
            return response_auth.is_success and await self.check_permissions(
    ...
    async def patch_resource(self, resource_name: str, **kwargs) -> None:
        """
    ...
    async def reset_user_pass(
        self, user_id: str, username: str, old_pass: str, new_pass: str
    ) -> None:
        """
        ...
        """
        auth_user = await self.get_token(
